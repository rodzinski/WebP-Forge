...
export function HistoryPanel({ entries, onClear, onClose }: HistoryPanelProps) {
    ...
    <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} onMouseDown={onClose}>
      <motion.section className="history-panel" role="dialog" aria-modal="true" aria-labelledby="history-title"
        ...
        <header className="report-header">
          <div><span>HISTÓRICO LOCAL</span><h2 id="history-title">Conversões recentes</h2><p>Até 50 lotes salvos somente neste navegador.</p></div>
          <button className="icon-button" onClick={onClose} aria-label="Fechar histórico">×</button>
        </header>
        {!entries.length ? <div className="history-empty"><strong>Nenhuma conversão registrada</strong><span>Os próximos lotes aparecerão aqui.</span></div> :
          <div className="history-layout">
            <div className="history-list">{entries.map((entry) => {
              ...
              return <button key={entry.id} className={entry.id === selected?.id ? "selected" : ""} onClick={() => setSelectedId(entry.id)}>
                <span>{new Date(entry.createdAt).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}</span>
            ...
            <div className="history-detail">
              ...
              <div className="history-items">{selected?.items.map((item) => <div key={item.id}>
                <span title={item.name}>{item.name}</span><b className={`status status-${item.status.toLowerCase().replace("í", "i")}`}>{item.status}</b>
                <small>{formatBytes(item.sourceSize)} → {item.outputSize === undefined ? "—" : formatBytes(item.outputSize)}{item.error ? ` · ${item.error}` : ""}</small>
              </div>)}</div>
            </div>
          </div>}
        <footer className="history-footer"><span>O histórico não contém as imagens e nunca é enviado.</span><button className="button ghost" onClick={onClear} disabled={!entries.length}>Limpar histórico</button></footer>
      </motion.section>
    </motion.div>
  );
}
